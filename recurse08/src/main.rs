...
impl Vm {
    ...
    fn find_var(&self, name: &str) -> Option<&Value> {
        self.vars.iter().rev().find_map(|vars| vars.get(name).map(|vars| var.to_owned()))
    }
}
...
fn eval(code: Value, vm: &mut Vm) {
    ...
    if let Value::Op(ref op) = code {
        let val = vm.find_var(op).expect(&format!("{op:?} is not a defined operation")).clone();
        match val {
            Value::Block(block) => {
...
mod test {
    ...
    #[test]
    fn test_group() {
        assert_eq!(parse("1 2 + { 3 4 }"), vec![Num(3), Block(vec![Num(3), Num(4)])]);
    }
