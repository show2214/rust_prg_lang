    ...
    fn call(&self, args: &[f64], frame: &StackFrame) -> f64 {
        match self {
            Self::User(code) => {
                ...
                new_frame.vars = args.iter().zip(code.args.iter())
                    .map(|(arg, name)| (name.to_string(), *arg)).collect();
...
fn binary_fn<'a>(f: fn(f64, f64) -> f64) -> FnDef<'a> {
    FnDef::Native(NativeFn {
        code: Box::new(move |args| {
            ...
            let lhs = args.next().expect("functions missing the first argument");
            let rhs = args.next().expect("functions missing the second argument");
            f(*lhs, *rhs)
        }),
    })
}
